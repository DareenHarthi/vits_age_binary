...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        return (text, spec, wav, sid, age, sex, dialect)

    def get_audio(self, filename):
        if 'timit' in filename:
            filename = filename.replace(".WAV", "_22050.wav")
        ...
        audio_norm = audio_norm.unsqueeze(0)
        if 'timit' in filename:
            spec_filename = filename.replace("_22050.wav", ".spec.pt")
        else:
            spec_filename = filename.replace(".wav", ".spec.pt")
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        if self.return_ids:
            return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, sid, age, sex, dialect, ids_sorted_decreasing
        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, sid, age, sex, dialect
